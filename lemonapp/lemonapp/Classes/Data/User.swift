...
final class User: Copying {
    ...
    init(id: Int,
        ...
        if let profilePhotoUrl = URL(string: profilePhoto?.removingPercentEncoding ?? "") {
            ...
            self._dataModel = dataModel ?? UserModel(id: id, firstName: firstName, lastName: lastName, email: email, mobilePhone: mobilePhone, profilePhoto: profilePhoto, defaultAddressId: defaultAddressId, defaultPaymentCardId: defaultPaymentCardId, walletAmount: walletAmount, referralCode: referralCode, isAdmin: isAdmin)
    }
...
extension User: DataModelWrapper {
    ...
    func syncDataModel() {
        ...
        _dataModel.isAdmin = self.isAdmin
