...
class ServiceViewDoubleLine: UIView {
    ...
    fileprivate static let lineHeightPadding: CGFloat = 10
    ...
    fileprivate func initialize() {
        let containerS = isSmallButton ? ServiceViewDoubleLine.containerSmallSize : ServiceViewDoubleLine.containerSize
        
        let lineHeight = self.isLabelVisible ? containerS : containerS - ServiceViewDoubleLine.titleHeight - ServiceViewDoubleLine.titleMarginTop
        let paddingHeight = self.lineCount == 1 ? CGFloat(0) : CGFloat(self.lineCount) * ServiceViewDoubleLine.lineHeightPadding
        
        if currentServiceView == nil {
            if let view = Bundle.main.loadNibNamed("ServiceView", owner: self, options: nil)![0] as? UIView {
                self.frame = CGRect(origin: self.frame.origin, size: CGSize(width: view.frame.width, height: view.frame.height - 151 + lineHeight * CGFloat(self.lineCount) + paddingHeight))
            ...
            if let items = self?.items.array, let lineCount = self?.lineCount {
                ...
                for i in 0..<lineCount {
                    ...
                    for j in 0..<items.count {
                        ...
                        let containerS = isSmall ? ServiceViewDoubleLine.containerSmallSize : ServiceViewDoubleLine.containerSize
                        
                        containerView.frame.size.width = containerS
                        containerView.frame.size.height = containerS
                        let paddingTop = i == 0 ? CGFloat(0) : ServiceViewDoubleLine.lineHeightPadding
                        containerView.frame.origin.y = lineHeight * CGFloat(i) + paddingTop
                        ...
                        button.frame.size.width = isSmall ? ServiceViewDoubleLine.buttonSmallSize : ServiceViewDoubleLine.buttonSize
                        button.frame.size.height = isSmall ? ServiceViewDoubleLine.buttonSmallSize : ServiceViewDoubleLine.buttonSize
                        ...
                        if isAttribute && items[j].selectedImage == nil {
                            ...
                            checkMark.frame.size.width = ServiceViewDoubleLine.checkMarkSize
                            checkMark.frame.size.height = ServiceViewDoubleLine.checkMarkSize
                            checkMark.frame.origin.x = (ServiceViewDoubleLine.buttonSize / 2) - (ServiceViewDoubleLine.checkMarkSize / 2)
                            checkMark.frame.origin.y = (ServiceViewDoubleLine.buttonSize / 2) - (ServiceViewDoubleLine.checkMarkSize / 2)
                        ...
                        if (self?.isRoundButtons ?? true) {
                            if items[j].isRectangle == nil || items[j].isRectangle == false {
                                button.cornerRadius = ServiceViewDoubleLine.buttonSize / 2
                            }
                        }
                        ...
                        //add touch long here
                        let holdGestureRecognizer = UILongPressGestureRecognizer(target: self, action: #selector(ServiceViewDoubleLine.showHoldAndEditEvent(_:)));
                        containerView.tag = j
                        ...
                        if (self?.isLabelVisible ?? true) {
                            ...
                            label.frame.origin.y = button.frame.size.height + ServiceViewDoubleLine.titleMarginTop
                            label.frame.size.width = button.bounds.width
                            label.frame.size.height = ServiceViewDoubleLine.titleHeight
                            label.textColor = UIColor.appBlueColor
                            label.font = label.font.withSize(11)
                            label.textAlignment = .center
                            containerView.addSubview(label)
                        }
                        let spacerS: CGFloat = isSmall ? 0 : ServiceViewDoubleLine.spacerSize
                        xPosition += ServiceViewDoubleLine.buttonSize + spacerS
                        scrollViewContentSize += ServiceViewDoubleLine.buttonSize + spacerS
        ...
        if canChangeVisibility {
            let longTap = UILongPressGestureRecognizer(target: self, action: #selector(ServiceViewDoubleLine.onTitleLongPress(_:)))
