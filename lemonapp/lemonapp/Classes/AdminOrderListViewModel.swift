...
class AdminOrderListViewModel: ViewModel {
        ...
        dashboardItems.observeNext { [weak self] event in
            ...
            default:
                if let strongSelf = self {
                    let dashboardViewModelsArray: [ViewModel] = strongSelf.dashboardItems.array.flatMap {
                        //return AdminOrderCellViewModel(order: $0 as! Order)
    ...
    func snoozeOrderFor(_ seconds: Int, order: Order) {
        ...
        if order.status == .awaitingPickup {
            newDate = order.delivery.estimatedPickupDate?.addingTimeInterval(Double(seconds)) as! Date ?? Date().addingTimeInterval(Double(seconds))
        } else {
            newDate = order.delivery.deliveryDate?.addingTimeInterval(Double(seconds)) as! Date ?? Date().addingTimeInterval(Double(seconds))
        }
    ...
    func reorderBySearchString(_ searchText: String) {
        let dashboardViewModelsArray:[ViewModel] = dashboardItems.array.flatMap {
            if let order = $0 as? Order {
    ...
    func update(_ completion: ( () -> Void )? = nil) {
        ...
        DataProvider.sharedInstance.refreshAdminOrders() { _ in completion?() }
