...
final class DeliveryViewController : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let viewModel = viewModel {
            ...
            promocodeView.isUserInteractionEnabled = !viewModel.isAdminMode
            addressField.isUserInteractionEnabled = !viewModel.isAdminMode
            paymentField.isUserInteractionEnabled = !viewModel.isAdminMode
        }
...
//    }
    ...
    func bindViewModel(_ viewModel: DeliveryViewModel) {
        ...
        completeButton.bnd_tap.observeNext { [weak self] in
            ...
            if viewModel.isAdminMode {
                if viewModel.hasChanges.value {
                    let alert = UIAlertController(title: "Are you sure you want to change this customer's order?", message: nil, preferredStyle: UIAlertControllerStyle.alert)
        ...
        if (viewModel.isAdminMode) {
            viewModel.hasChanges.observeNext { [weak self] value in
                self?.completeButton.setTitle(value ? "Update Order" : "Done")
            }
        }
        ...
        self.orderDetailsButton.isEnabled = viewModel.isAdminMode
        self.orderDetailsButton.tintColor = viewModel.isAdminMode ? UIColor.white : UIColor.clear
    ...
    @objc func didOrderDetailsButtonClicked(_ sender: AnyObject?) {
        if let viewModel = self.viewModel, viewModel.isAdminMode == true {
