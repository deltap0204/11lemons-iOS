...
final class ReceiptViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if let order = order {
            self.viewModel = ReceiptViewModel(order: order, delegate: self, isAdmin: self.isAdmin ?? false)
        }
        ...
        if let viewModel = viewModel {
            viewModel.totalPrice.map{ String(format: "Total: $%.2f", $0) }.bind(to: totalBundle.bnd_title).dispose(in:self.disposeBag)
    ...
    fileprivate func initFeedback() -> UIView {
        ...
        let view = FeedbackView(isAdmin: true)
        view.viewModel = FeedbackViewModel(text: viewModel.order.feedbackText ?? "", rate: viewModel.order.feedbackRating ?? 0, isAdmin: true )
    ...
    fileprivate func showPaymentsOptions() {
        guard let viewModel = self.viewModel,
            viewModel.order.paymentStatus != .paymentProcessedSuccessfully,
            !viewModel.isAdmin else {return}
    ...
    fileprivate func setupBtnDone() {
        if let viewModel = self.viewModel {
            let btnTitle = viewModel.isAdmin ? "Process Payment" : "Done"
            btnDone.setTitle(btnTitle)
        }
...
extension ReceiptViewController: MGSwipeTableCellDelegate {
    func swipeTableCell(_ cell: MGSwipeTableCell, canSwipe direction: MGSwipeDirection, from point: CGPoint) -> Bool {
        guard let viewModel = self.viewModel, viewModel.isAdmin else { return false }
    ...
    func swipeTableCell(_ cell: MGSwipeTableCell, tappedButtonAt index: Int, direction: MGSwipeDirection, fromExpansion: Bool) -> Bool {
        guard let viewModelCell = (cell as? ReceiptItemCell)?.viewModel else {
            return true
        }
        
        switch index {
        case 0:
            self.viewModel?.showDeleteConfirmation(self, viewModel: viewModelCell)
            break
        default:
            break
        }
