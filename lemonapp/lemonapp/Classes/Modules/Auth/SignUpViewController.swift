    ...
    var signUpRequest: Action<SingUpResolver> {
        return Action { [weak self] in
            Signal/*(replayLength: 1)*/ { [weak self] sink in
                ...
                let isValid =  strongSelf.validation()
                if isValid {
                    if let firstNameVal = strongSelf.firstName.value,
                        ...
                        let zipCodeVal = strongSelf.zipCode.value {
                            _ = LemonAPI.validateZip(zip: zipCodeVal).request().observeNext { (resolver: EventResolver<[LocationData]>) in
    ...
    fileprivate func signUp() {
        ...
        viewModel.signUpRequest.execute { [weak self] resolver in
            do {
                ...
                if !location.allowReg {
                    (UIApplication.shared.delegate as? AppDelegate)?.window?.rootViewController?.handleError(LemonError.unavailableLocation)
                }
