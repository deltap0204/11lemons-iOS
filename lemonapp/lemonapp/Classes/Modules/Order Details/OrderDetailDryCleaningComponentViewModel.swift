//
//  OrderDetailDryCleaningComponentViewModel.swift
//  lemonapp
//
...
class OrderDetailDryCleaningComponentViewModel: ComponentsViewModel {
    ...
    override func getPreferencesOptions() -> [PreferenceRowUser] {
        let solutionRow = SolutionPreferenceRowViewModel(order: order, delegate: self)
        let packagingRow = PackagingPreferenceRowViewModel(order: order, delegate: self)
        
        return [solutionRow, packagingRow]
    }
