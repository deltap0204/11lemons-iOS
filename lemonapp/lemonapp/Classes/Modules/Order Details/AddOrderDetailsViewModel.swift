...
final class AddOrderDetailsViewModel {
    ...
    func createOrderDetail() {
        ...
        if let orderDetail = orderDetailToAdd.value {
            orderDetail.orderId = self.order?.id ?? 0
            
            if orderDetail.garment == nil {
                orderDetail.garment = OrderGarment(id: 0)
                orderDetail.garment?.orderId = order?.id  ?? 0
            }
    ...
    func getUserNameCreator() -> String {
        if let order = order, let user = order.createdBy {
    ...
    func updateOrders() {
        DataProvider.sharedInstance.refreshAdminOrders() { [weak self] _ in
