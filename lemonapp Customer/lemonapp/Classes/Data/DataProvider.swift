...
//    }
    ...
    func restoreData(_ complite:(() -> ())? = nil) {
        ...
        LemonCoreDataManager.fetchAsync(UserModel.self) { [weak self] in
            ...
            do {
                let users = try $0() ?? []
                if let userLoggedID = UserDefaults.standard.object(forKey: LemonAPI.USER_ID_HEADER) as? Int {
                    let integerLiteral = NSNumber(integerLiteral: userLoggedID)
                    if let userModel =  users.first(where: {$0.id == integerLiteral }) {
    ...
    fileprivate func getDashboardItems() -> [DashboardItem] {
        var orders: [Order] = self.userOrders.array.flatMap { $0.status == .canceled || $0.status == .archived ? nil : $0 }
